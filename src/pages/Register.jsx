...
import { User, Mail, Phone, Lock, CreditCard, ChevronRight, CheckCircle2, Building2 } from 'lucide-react';
import { supabase } from '../lib/supabaseClient';

const PRICE_PER_UNIT = 15; // € pro m²
...
const Register = () => {
    ...
    const validateIBAN = (iban) => {
        return /^[A-Z]{2}[0-9]/i.test(iban.replace(/\s/g, ''));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateIBAN(formData.iban)) {
            alert("Bitte eine valide IBAN eingeben (z.B. DE...)");
            return;
        }
        if (!formData.mandate_accepted) {
            alert("Bitte das SEPA-Lastschriftmandat akzeptieren.");
            return;
        }
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center space-x-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                                ...
                                <div className="text-gray-400 text-sm font-medium uppercase tracking-wider">pro Quadratmeter</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex bg-gray-100 rounded-xl p-1 mb-3">
                                    <button
                                        ...
                                        className={`flex-1 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all ${(formData.inputMode || 'sqm') === 'sqm' ? 'bg-[#0c151a] text-white shadow-lg' : 'text-gray-500'}`}
                                    >
                                        m² auswählen
                                    </button>
                                    <button
                                ...
                                <label className="flex items-start space-x-3 cursor-pointer group">
                                    <div className="relative mt-1">
                                        <input
                                            ...
                                            onChange={(e) => setFormData({ ...formData, mandate_accepted: e.target.checked })}
                                        />
                                        <div className="w-5 h-5 border-2 border-gray-300 rounded peer-checked:bg-[#0c151a] peer-checked:border-[#0c151a] transition-all" />
                                        <CheckCircle2 className="absolute top-0 left-0 w-5 h-5 text-white opacity-0 peer-checked:opacity-100 transition-opacity p-0.5" />
                                    </div>
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    <span>Jetzt Spenden: {(formData.sq_meters * PRICE_PER_UNIT).toFixed(2)}€</span>
